...
cnnlHandle_t &sycl_bang_stream_t::get_cnnl_handle(CNqueue bang_stream) {
    ...
    e->activate_stream_cnnl(bang_stream);
    return *(e->get_cnnl_handle());
}
// the sycl_bang_stream_t will not own this. it is an observer pointer
CNqueue sycl_bang_stream_t::get_underlying_stream() {
    return compat::get_native<CNqueue>(*queue_);
}
...
// the sycl_bang_stream_t will not own this. it is an observer pointer
CNdev sycl_bang_stream_t::get_underlying_device() {
    return compat::get_native<CNdev>(queue_->get_device());
}
...
status_t sycl_bang_stream_t::interop_task(
        std::function<void(::sycl::handler &)> sycl_bang_interop_) {
    try {
        auto event = queue().submit([&](::sycl::handler &cgh) {
            cgh.depends_on(sycl_ctx().get_sycl_deps().events);
            sycl_bang_interop_(cgh);
        });
