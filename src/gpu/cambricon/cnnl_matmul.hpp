...
#ifndef GPU_CAMBRICON_CNNL_MATMUL_HPP
#define GPU_CAMBRICON_CNNL_MATMUL_HPP
...
struct cnnl_matmul_t : public primitive_t {
    ...
    status_t init(engine_t *engine) override {
        matmul_impl_.reset(new cnnl_matmul_impl_t());
        const auto status
        ...
        if (has_runtime_args && with_bias) {
            executor_.reset(new cnnl_matmul_runtime_args_bias_exec_t);
        } else if (has_runtime_args) {
            executor_.reset(new cnnl_matmul_runtime_args_exec_t);
        } else if (with_bias) {
            executor_.reset(new cnnl_matmul_bias_exec_t);
        } else {
            executor_.reset(new cnnl_matmul_exec_t);
        }
    ...
    std::shared_ptr<cnnl_matmul_impl_t> matmul_impl_;
    std::shared_ptr<cnnl_matmul_exec_base_t> executor_;
